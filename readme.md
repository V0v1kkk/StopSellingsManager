...
4. Убедиться, что текст в файлах **open.txt** и **close.txt** в папке **WorkFolder** соответствует
    ...
    > В тексте можно размещать символы юникода, вставляя их шестнадцатеричные представления. Например: _\u26D4_ .
...
> Данные обновляются с интервалом в 5 минут.
...
с именем **othet.csv**. В случае удаления/перемещения файла отчета закрытии следующей
...
> В сдучае сбоя достаточно удалить файы **StopSellings.dat** и **owner.txt** в рабочей папке.
...
4. Вставить, скопированный из SSD, номер обращения с датой и временем в поле **4**
